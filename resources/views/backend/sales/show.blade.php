            ...
            <div class="row gutters-5">
                <div class="col text-md-left text-center">
                    @if(json_decode($order->shipping_address))
                        <address>
                            <strong class="text-main">Name: {{ ucfirst(json_decode($order->shipping_address)->name) }}</strong><br>
                            Email: {{ json_decode($order->shipping_address)->email }}<br>
                            Phone: {{ json_decode($order->shipping_address)->phone }}<br>
                            Address: {{ json_decode($order->shipping_address)->address }}, @if(isset(json_decode($order->shipping_address)->state)) {{ json_decode($order->shipping_address)->state }} - @endif {{ json_decode($order->shipping_address)->postal_code ?? '' }}<br>
                            Shipping to: {{ $order->shipping_charge->name }}<br />
                        </address>
                    @else
                        <address>
