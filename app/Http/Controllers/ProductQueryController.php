...
class ProductQueryController extends Controller
{
    ...
    {
        $query = ProductQuery::find($request->id);
        if ($query) {
            $query->delete();
            flash(translate('Query deleted successfully'))->success();
        } else {
            flash(translate('Query not found'))->error();
        }
        return redirect()->route('product_query.index');
    }
    ...
    {
        $product_query = ProductQuery::findOrFail($request->id);
        $product_query->published = $request->status;
        $product_query->save();
        return 1;
    }
}
