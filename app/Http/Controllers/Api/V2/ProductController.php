...
use App\Http\Resources\V2\ProductMicroCollection;
...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        if($request->filled('category')){
            $category = Category::with('childrenCategories')->where('slug', $request->category)->first();
            if($category == null){
            ...
            $products = filter_products($category->products());
            if(!$products->exists()){
                return response()->json([
                    'success' => false,
                    'status' => 404,
    ...
    {
        $product = Product::with(['faqs.faq_translations', 'faqs.products'])->where('id', $idOrSlug)
    ...
    {
        $product = Product::with(['faqs.faq_translations', 'faqs.products'])->where('slug', $slug)->first();
    ...
    public function categoryProducts($slug, Request $request)
    {
        ...
        if($category == null){
            return response()->json(['status' => 'error', 'message' => 'Category not found'], 200);
        }
    ...
    {
        $products = Product::where('upcoming', 1)->physical();
        return new ProductMiniCollection(filter_products($products)->latest()->paginate(10));
    }
    ...
    public function thirtyDays()
    {
        ...
        ->where('created_at', '<=', Carbon::now())->physical();
        return new ProductMiniCollection(filter_products($products)->latest()->paginate(10));
    }
    ...
    public function microSearch(Request $request)
    {
        ...
        $products = Product::where('name', 'like', '%' . $request->q . '%')->physical();
        return new ProductMicroCollection(filter_products($products)->latest()->paginate(10));
    }
    ...
    public function search(Request $request)
    {
        ...
        // Filter by made in and product type
        if ($request->has('made_in') && $request->made_in != "") {
            $made_in = explode(',', $request->made_in);
            $products->whereIn('country_id', $made_in);
        }

        // Filter by made in and product type
        if ($request->has('product_type') && $request->product_type != "") {
            $product_type = explode(',', $request->product_type);
            $products->whereIn('product_type_id', $product_type);
        }

        //attribute filtering
        if ($request->has('attribute_ids') && $request->attribute_ids != "") {
            $attribute_ids = explode(',', $request->attribute_ids);

            $products->where(function ($query) use ($attribute_ids) {
                foreach ($attribute_ids as $id) {
                    $query->orWhereJsonContains('attributes', (string) $id);
                }
            });
        }
