    ...
    public function store(array $data, $product)
    {
        ...
        if (count($combinations) > 0) {
            ...
            foreach ($combinations as $key => $combination) {
                ...
                $product_stock->discount = request()['discount_' . str_replace('.', '_', $str)] ?? 0;
                $product_stock->sku = request()['sku_' . str_replace('.', '_', $str)] ?? 0;
                $product_stock->qty = request()['qty_' . str_replace('.', '_', $str)] ?? 0;
                $product_stock->image = request()['img_' . str_replace('.', '_', $str)] ?? null;
                $product_stock->save();
            }
        } else {
