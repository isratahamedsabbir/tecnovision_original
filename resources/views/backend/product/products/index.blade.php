            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        {{-- <th data-breakpoints="lg">{{translate('Set On Home Page')}}</th> --}}
                        <th data-breakpoints="lg">{{translate('Upcoming')}}</th>
                        <th data-breakpoints="lg">{{translate('Best Seller')}}</th>
                        <th data-breakpoints="lg">Created</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="row gutters-5 w-200px w-md-300px mw-100">
                                ...
                                <div class="col">
                                    <span class="text-muted text-truncate-2">{{ Str::limit($product->getTranslation('name'), 20, ' ...') }}</span>
                                </div>
                            </div>
                        </td>
                        ...
                        {{-- <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_home(this)" value="{{ $product->id }}" type="checkbox" <?php if ($product->on_homepage == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td> --}}
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_upcoming(this)" value="{{ $product->id }}" type="checkbox" <?php if ($product->upcoming == 1) echo "checked"; ?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td>
                            ...
                            <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                                <div class="btn-group" role="group">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                        <a class="dropdown-item" href="{{ config('app.frontend') }}/product/{{ $product->slug }}" target="_blank" title="{{ translate('View') }}"><i class="las la-eye"></i>&nbsp;View</a>
                                        @can('product_edit')
                                            @if ($type == 'Seller')
                                                <a class="dropdown-item" href="{{route('products.seller.edit', ['id'=>$product->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                                                    <i class="las la-edit"></i>
                                                    &nbsp;Edit
                                                </a>
                                            @else
                                                <a class="dropdown-item" href="{{route('products.admin.edit', ['id'=>$product->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                                                    <i class="las la-edit"></i>
                                                    &nbsp;Edit
                                                </a>
                                            @endif
                                            @can('product_delete')
                                                <a href="#" class="dropdown-item confirm-delete" data-href="{{route('products.destroy', $product->id)}}" title="{{ translate('Delete') }}">
                                                    <i class="las la-trash"></i>
                                                    &nbsp;Delete
                                                </a>
                                            @endcan
...
@section('script')
    <script type="text/javascript">
        ...
        function update_upcoming(el){
            ...
            $.post("{{ route('products.upcoming') }}", {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', "{{ translate('Upcoming products updated successfully') }}");
                }
                else{
                    AIZ.plugins.notify('danger', "{{ translate('Something went wrong') }}");
                }
        ...
        function update_best_seller(el){
            ...
            $.post("{{ route('products.best_seller') }}", {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
        ...
        function update_home(el){
            ...
            $.post("{{ route('products.on_homepage') }}", {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
