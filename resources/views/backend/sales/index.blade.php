                                    ...
                                    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                                        <div class="btn-group" role="group">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                <a class="dropdown-item" href="{{ $order_detail_route }}" title="{{ translate('View') }}"><i class="las la-eye"></i> View</a>
                                                <a class="dropdown-item" href="{{ route('invoice.download', $order->id) }}" title="{{ translate('Download Invoice') }}"><i class="las la-download"></i> Invoice</a>
                                                @if(auth()->user()->can('unpaid_order_payment_notification_send') && $order->payment_status == 'unpaid' && $unpaid_order_payment_notification->status == 1)
                                                    <a class="dropdown-item" href="javascript:void();" onclick="unpaid_order_payment_notification('{{ $order->id }}');" title="{{ translate('Unpaid Order Payment Notification') }}"><i class="las la-bell"></i> Notification</a>
                                                @endif
                                                @can('delete_order')
                                                    <a class="dropdown-item confirm-delete" href="#" data-href="{{ route('orders.destroy', $order->id) }}" title="{{ translate('Delete') }}"><i class="las la-trash"></i> Delete</a>
                                                @endcan
                                            </div>
                                        </div>
                                    </div>
